...
- **Training data preparation**: Combine with [model-trainer.md](/model-compose/zh-cn/reference/compose/components/model-trainer.md) to feed fine-tuning jobs
- **Evaluation set assembly**: Load test/validation splits and forward to evaluators
- **Dataset transformation**: Pre-process before vectorization for [vector-store.md](/model-compose/zh-cn/reference/compose/components/vector-store.md)
